...
df_acc_ears <- data.frame(
  ...
  true_bed <- as.character(),
  false_bed <- as.character(),
  true_rise <- as.character(),
  false_rise <- as.character(),
...
df_temp_j <-
  df_temp_j %>% 
  dplyr::mutate(
    true_bed = as.numeric(ifelse((diary_bed == 1 & ears_bed == 1), 1, 0)),
    false_bed = as.numeric(ifelse((is.na(diary_bed) & ears_bed == 1), 1, 0)),
    true_rise = as.numeric(ifelse((is.na(diary_bed) & is.na(ears_bed)), 1, 0)),
    false_rise = as.numeric(ifelse((diary_bed == 1 & is.na(ears_bed)), 1, 0))
    )
...
df_temp_j_agg <- df_temp_j %>% 
  ...
  dplyr::summarise(
    true_bed = sum(true_bed, na.rm = TRUE),
    false_bed = sum(false_bed, na.rm = TRUE),
    true_rise = sum(true_rise, na.rm = TRUE),
    false_rise = sum(false_rise, na.rm = TRUE)
  ) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(
    ears.diary_bed_sensitivity = true_bed/(true_bed + false_bed),
    ears.diary_bed_specificity = true_rise/(false_rise + true_rise),
    ears.diary_bed_accuracy = 
      (true_bed + true_rise) / (true_bed + false_bed + true_rise + false_rise)
    )
...
for(i in seq_along(unique_actilife_id)){
  ...
  for(j in seq_along(unique_day_num)){
    ...
    df_temp_j <- 
      df_temp_j %>% 
      dplyr::mutate(
        true_bed = as.numeric(ifelse((actilife_bed == 1 & ears_bed == 1), 1, 0)),
        false_bed = as.numeric(ifelse((is.na(actilife_bed) & ears_bed == 1), 1, 0)),
        true_rise = as.numeric(ifelse((is.na(actilife_bed) & is.na(ears_bed)), 1, 0)),
        false_rise = as.numeric(ifelse((actilife_bed == 1 & is.na(ears_bed)), 1, 0))
      )
    ...
    df_temp_j_agg <-
      ...
      dplyr::ungroup() %>% 
      dplyr::mutate(
        ears.actilife_bed_sensitivity = true_bed/(true_bed + false_bed),
        ears.actilife_bed_specificity = true_rise/(false_rise + true_rise),
        ears.actilife_bed_accuracy = (true_bed + true_rise) / (true_bed + false_bed + true_rise + false_rise)
...
df_temp_j_agg_trunc <- 
  ...
  dplyr::ungroup() %>% 
  dplyr::mutate(
    ears.diary_bed_sensitivity_trunc = true_bed/(true_bed + false_bed),
    ears.diary_bed_specificity_trunc = true_rise/(false_rise + true_rise),
...
df_temp_j_supp <-
  df_temp_j_supp %>% 
  dplyr::mutate(
    true_sleep = as.numeric(ifelse((diary_sleep == 1 & ears_bed == 1), 1, 0)),
    false_sleep = as.numeric(ifelse((is.na(diary_sleep) & ears_bed == 1), 1, 0)),
    true_wake = as.numeric(ifelse((is.na(diary_sleep) & is.na(ears_bed)), 1, 0)),
    false_wake = as.numeric(ifelse((diary_sleep == 1 & is.na(ears_bed)), 1, 0))
    )
...
ggsave(filename = "SupplementalFig1.tiff", width = 12, height = 12)
```
