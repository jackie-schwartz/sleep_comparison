...
## Participants
From my analysis:
29 University students provided smartphone accelerometer and self-reported diary data for one week. 18 participants provided actigraphy data and one participant was excluded for having only one day of data, leaving 17 participants with at least 5 days of data.
## Procedure
...
```{r}
load("reliability_June272024.Rda")
## Reading in files
...
ggsave("Figure2.tiff", width = 14, height = 8)
...
(de_corr_onset_supp + de_corr_offset_supp + de_corr_dur_supp) +
  plot_annotation(title = 'Intersource Correlations', tag_levels = 'A')
ggsave("SupplementalFig6.tiff", width = 14, height = 4)
...
# _ Sample 1 EARS v DIARY Spec/Sens:  ----

ears_spec_sens <- 
  df_analysis_wide3_RemOut %>% 
  filter(sample_1_flag == 1) %>%
  dplyr::select(actilife_id, risetime_date, day_num, sample_1_flag, ends_with(c("bed_sensitivity", "bed_specificity", "bed_accuracy"))) %>%
  drop_na(ears.diary_bed_sensitivity)
...
ears_spec_sens %>% # specificty and sensitivity are against the diary data (this is more intuitive than the individual participant distributions)
  ggplot() + 
 geom_line(aes(x = 1 - ears.diary_bed_specificity, y = ears.diary_bed_sensitivity), linewidth = 1.5, color = "#56B4E9", alpha = .5) +
  geom_point(aes(x = 1 - ears.diary_bed_specificity, y = ears.diary_bed_sensitivity), linewidth = 1.5, color = "#56B4E9", alpha = .5) +
...
ggsave("Figure3.tiff", width = 6, height = 6)

mode(1 - ears_spec_sens$ears.diary_bed_specificity)
mode(ears_spec_sens$ears.diary_bed_sensitivity)

# EARS v DIARY (1440 min/day)
ears_sens_spec_acc <- 
  ears_spec_sens %>%
  dplyr::ungroup() %>%
  dplyr::summarise(
    sensitivity = mean(ears.diary_bed_sensitivity), # TPR
    specificity = mean(ears.diary_bed_specificity),
    accuracy = mean(ears.diary_bed_accuracy),
    fpr = mean(1 - ears_spec_sens$ears.diary_bed_specificity, na.rm=TRUE)
  ) 

################ SUPPLEMENTARY ################ 

# _ Sample 1 Spec/Sens: EARS BED v DIARY SLEEP (1440)----

ears_spec_sens_supp <- 
  df_analysis_wide3_RemOut %>% 
  filter(sample_1_flag == 1) %>% 
  dplyr::select(actilife_id, risetime_date, day_num, sample_1_flag, ends_with(c("sleep_sensitivity", "sleep_specificity", "sleep_accuracy"))) %>% 
  drop_na(ears.diary_sleep_sensitivity)

ears_spec_sens_supp %>% 
  ggplot() + 
 geom_line(aes(x = 1 - ears.diary_sleep_specificity, y = ears.diary_sleep_sensitivity), linewidth = 1.5, color = "#56B4E9", alpha = .5) +
  geom_point(aes(x = 1 - ears.diary_sleep_specificity, y = ears.diary_sleep_sensitivity), linewidth = 1.5, color = "#56B4E9", alpha = .5) +
  geom_abline(lty = 2, alpha = 0.5, color = "gray50", linewidth = 1.2) +
  expand_limits(x = c(0,1), y = c(0,1)) +
  theme_minimal() +
  labs(
...
mode(1 - ears_spec_sens_supp$ears.diary_sleep_specificity) # 0
mode(ears_spec_sens_supp$ears.diary_sleep_sensitivity) # 1

ears_spec_sens_acc_supp <- 
  ears_spec_sens_supp %>%
  dplyr::ungroup() %>%
  dplyr::summarise(
    sensitivity = mean(ears.diary_sleep_sensitivity), # TPR
    specificity = mean(ears.diary_sleep_specificity),
    accuracy = mean(ears.diary_sleep_accuracy),
    fpr = mean(1 - ears_spec_sens_supp$ears.diary_sleep_specificity, na.rm=TRUE)
  ) 


# _ Sample 1 EARS v DIARY Spec/Sens TRUNCATED: EARS (8:30-10:39) v Diary ----

ears_spec_sens_trunc <- 
  df_analysis_wide3_RemOut %>% 
  filter(sample_1_flag == 1) %>%
  dplyr::select(actilife_id, risetime_date, day_num, sample_1_flag, ends_with(c("bed_sensitivity_trunc", "bed_specificity_trunc", "bed_accuracy_trunc"))) %>%
  drop_na(ears.diary_bed_sensitivity_trunc)


mode(1 - ears_spec_sens_trunc$ears.diary_bed_specificity_trunc) # 0
mode(ears_spec_sens_trunc$ears.diary_bed_sensitivity_trunc) # 1

ears_spec_sens_trunc_acc <- 
  ears_spec_sens_trunc %>%
  dplyr::ungroup() %>%
  dplyr::summarise(
    sensitivity = mean(ears.diary_bed_sensitivity_trunc), # TPR
    specificity = mean(ears.diary_bed_specificity_trunc),
    accuracy = mean(ears.diary_bed_accuracy_trunc),
    fpr = mean(1 - ears_spec_sens_trunc$ears.diary_bed_specificity_trunc, na.rm=TRUE)
  ) 


# _ Sample 2 EARS v ACTIGRAPH Spec/Sens:  ----

# EARS v ActiGraph
act_spec_sens <-
  df_analysis_wide3_RemOut %>% 
  filter(sample_2_flag == 1) %>%
  dplyr::select(actilife_id, risetime_date, day_num, sample_2_flag, ends_with(c("bed_sensitivity", "bed_specificity", "bed_accuracy"))) %>%
  drop_na(ears.actilife_bed_accuracy)

mode(1 - act_spec_sens$ears.actilife_bed_specificity) # 0
mode(act_spec_sens$ears.actilife_bed_sensitivity) # 1

act_spec_sens_acc <- 
  act_spec_sens %>%
  dplyr::ungroup() %>%
  dplyr::summarise(
    sensitivity = mean(ears.actilife_bed_sensitivity), # TPR
    specificity = mean(ears.actilife_bed_specificity),
    accuracy = mean(ears.actilife_bed_accuracy),
    fpr = mean(1 - act_spec_sens$ears.actilife_bed_specificity, na.rm=TRUE)
  ) 
...
ggsave("SupplementalFig5.tiff", width = 7, height = 9)
